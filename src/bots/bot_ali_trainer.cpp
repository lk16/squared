...
std::vector<bot_ali*> bot_ali_trainer::procreate(const bot_ali* father, int n_children){
  ...
  for(int k=0;k<n_children;k++){
    bot_ali* child = new bot_ali();
    child->set_search_depth(train_depth, train_optimal_depth);
    res.push_back(child);
...
void bot_ali_trainer::run(){
  ...
  bot_ali* ali = new bot_ali();
  ali->set_search_depth(train_depth,train_optimal_depth);
  
  
  player_list_t scores(1,std::make_pair(ali, 0));
