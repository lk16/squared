...
bits64 board::do_move_internal_in_cpp()
{
  ...
  for(int dir=0;dir<8;++dir){
    if(valid_dir[dir]){
      bits64 line = dir_mask[dir][field_id] & bits64_reset[field_id];
      if(line & me){
        int first = bits64_find_first(line & me);
        int last = bits64_find_last(line & me);
        int chosen = find_first[dir] ? first : last;
        bits64 tmp = (line ^ dir_mask[dir][chosen]);
        if((opp & tmp) == tmp){
          flipped |= tmp;
        }
      }
...
const bits64 board::dir_mask[8][64] = {
  {
    0x0,
    0x0,
    0x0,
